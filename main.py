...
# --- 1. ТОКЕНИ ТА НАЛАШТУВАННЯ (З ЗАХИСТОМ ВІД NONE) ---
...
def webhook():
    if request.headers.get('content-type', '').startswith('application/json'):
        ...
        # Для сумісності з будь-якими запитами Telegram
        try:
            json_string = request.get_data().decode('utf-8')
            if json_string:
                update = telebot.types.Update.de_json(json_string)
            ...
            print(f"Помилка декодування вебхуку: {e}")
        return 'OK', 200
...
# --- 7. АВТОМАШТИЧНА РЕЄСТРАЦІЯ ВЕБХУКУ ---
def setup_webhook():
    try:
