...
# --- 2. ІНІЦІАЛІЗАЦІЯ БАЗИ ДАНИХ (З ЦІНАМИ) ---
def init_db():
...
# --- 3. ДАНІ ТА СЛОВНИКИ (З ВКАЗАННЯМ ЦІН) ---
SERVICES = {
    "☕️ Кавовий сет та десерт": {"price": 150, "time": "15 хв"},
    "🥐 Сніданок і Напій": {"price": "220 грн", "time": "20 хв"},
    "🍵 Чайна церемонія": {"price": "180 грн", "time": "30 хв"}
}

# Допоміжна функція для вилучення чистого числа ціни
...
MASTERS = ["Бариста Анна", "Бариста Максим", "Будь-хто вільний"]
...
def index():
    return "✅ Booking Bot Server with Financials is Running smoothly!", 200

# --- 5. КЛАВІАТУРИ ---
def main_keyboard(user_id):
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(
        types.KeyboardButton("📅 Онлайн-замовлення"),
        types.KeyboardButton("💰 Меню та Ціни"),
...
def admin_keyboard():
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    markup.add(
        types.KeyboardButton("📋 Активні замовлення"),
...
def show_admin_panel(message):
    ...
    safe_send_message(
        message.chat.id,
        "<b>👑 Панель Адміністратора</b>\n\nОберіть розділ керування та аналітики:",
        parse_mode="HTML",
        reply_markup=admin_keyboard()
    )

@bot.message_handler(func=lambda msg: msg.text == "📋 Активні замовлення")
def admin_show_appointments(message):
    ...
    if not apps:
        safe_send_message(message.chat.id, "Зараз немає активних замовлень.", reply_markup=admin_keyboard())
        return

    safe_send_message(message.chat.id, f"<b>📋 Всього активних замовлень: {len(apps)}</b>\n", parse_mode="HTML")
    for app_item in apps:
        app_id, user_name, service, price, master, date, time_slot, contact = app_item
        text = (
            f"🆔 <b>Замовлення #{app_id}</b>\n"
            f"👤 <b>Клієнт:</b> {user_name}\n"
            f"📞 <b>Контакт:</b> {contact}\n"
            f"📌 <b>Послуга:</b> {service} ({price})\n"
            f"💈 <b>Виконавець:</b> {master}\n"
            f"📅 <b>Дата/Час:</b> {date} о {time_slot}"
        )
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("❌ Скасувати це замовлення", callback_data=f"cancel_app_{app_id}"))
...
def admin_show_revenue(message):
    ...
    text = (
        "<b>💰 Фінансовий звіт закладу:</b>\n\n"
        f"🟢 <b>Потенційний дохід (активні замовлення):</b> <code>{active_revenue} грн</code>\n"
        f"📈 <b>Загальний обіг (включно зі скасованими):</b> <code>{total_revenue} грн</code>\n\n"
        "<i>💡 Розрахунок формується на основі прайсу замовлень.</i>"
    )
...
def admin_export_csv(message):
    ...
    # Генеруємо CSV файл у пам'яті
    output = io.StringIO()
    writer = csv.writer(output, delimiter=';')
    # Заголовки таблиці
    writer.writerow(["ID", "Клієнт", "Послуга", "Ціна", "Виконавець", "Дата", "Час", "Контакт", "Статус", "Створено"])
    
    for row in rows:
        writer.writerow(row)

    output.seek(0)
    file_bytes = io.BytesIO(output.getvalue().encode('utf-8-sig')) # utf-8-sig для коректного відкриття в Excel
    file_bytes.name = "booking_report.csv"

    bot.send_document(
        message.chat.id, 
        file_bytes, 
        caption="📊 <b>Ось таблиця з усіма даними та фінансами закладу.</b> Відкривається у Excel чи Google Таблицях.",
        parse_mode="HTML",
        reply_markup=admin_keyboard()
    )
...
# --- КЛІЄНТСЬКА ЧАСТИНА ---
@bot.message_handler(func=lambda msg: msg.text == "💰 Меню та Ціни")
def show_price(message):
    text = "<b>💵 Актуальне меню та ціни:</b>\n\n"
    for s_name, info in SERVICES.items():
        text += f"• <b>{s_name}</b>\n  Вартість: <code>{info['price']}</code> | Час: {info['time']}\n\n"
...
def show_cabinet(message):
    ...
    if not apps:
        safe_send_message(message.chat.id, "У вас немає активних замовлень.", reply_markup=main_keyboard(message.chat.id))
        return

    safe_send_message(message.chat.id, f"<b>👤 Ваші активні замовлення ({len(apps)}):</b>", parse_mode="HTML")
    for app_item in apps:
        app_id, service, master, date, time_slot = app_item
        text = f"📌 <b>{service}</b>\n👤 Виконавець: {master}\n📅 {date} о {time_slot}"
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("❌ Скасувати замовлення", callback_data=f"cancel_app_{app_id}"))
        safe_send_message(message.chat.id, text, parse_mode="HTML", reply_markup=markup)
...
def cancel_appointment_callback(call):
    try:
        ...
        bot.answer_callback_query(call.id, text="Замовлення успішно скасовано!")
        bot.edit_message_text(
            "❌ <b>Це замовлення було скасовано.</b>", 
...
@bot.message_handler(func=lambda msg: msg.text == "📅 Онлайн-замовлення")
def start_booking(message):
    ...
    safe_send_message(message.chat.id, "<b>Крок 1/5:</b> Оберіть позицію:", parse_mode="HTML", reply_markup=markup)
...
def step_select_master(message):
    ...
    markup.add(types.KeyboardButton("❌ Скасувати"))
    safe_send_message(message.chat.id, "<b>Крок 2/5:</b> Оберіть фахівця:", parse_mode="HTML", reply_markup=markup)
...
def finish_booking(message):
    ...
    safe_send_message(chat_id, f"🎉 <b>УСПІШНЕ ЗАМОВЛЕННЯ!</b>\n\n📌 {service} ({price})\n👤 Виконавець: {master}\n📅 Дата: {date} о {booking_time}", parse_mode="HTML", reply_markup=main_keyboard(chat_id))

    if ADMIN_CHAT_ID:
        admin_msg = f"⚡️ <b>НОВЕ ЗАМОВЛЕННЯ!</b>\n📌 {service} — <code>{price}</code>\n👤 Виконавець: {master}\n📅 {date} о {booking_time}\n📞 {contact}\n💬 Клієнт: {username}"
        safe_send_message(ADMIN_CHAT_ID, admin_msg, parse_mode="HTML")

def cancel_flow(message):
    bot.clear_step_handler_by_chat_id(chat_id=message.chat.id)
    safe_send_message(message.chat.id, "Замовлення скасовано.", reply_markup=main_keyboard(message.chat.id))
