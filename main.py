...
# --- НАЛАШТУВАННЯ ТА ЗМІННІ (З ФОЛБЕКОМ ДЛЯ ТОКЕНА) ---
...
# --- СТАРТ ТА ГОЛОВНІ РОЗДІЛИ ---
@bot.message_handler(commands=['start', 'menu'])
def send_welcome(message):
    bot.send_message(
        message.chat.id,
        f"Вітаємо у нашому сервісі, <b>{message.from_user.first_name}</b>! 👋\n\n"
