...
def get_phone(message):
    ...
    name = user_data.get(message.chat.id, {}).get("name", "Не вказано")
    service = user_data.get(message.chat.id, {}).get("service", "Не вказано")
    phone = message.text
...
if __name__ == "__main__":
    # Запускаємо Telegram бота окремим фоновим потоком
    threading.Thread(target=start_bot, daemon=True).start()
    
    # Запускаємо Flask веб-сервер у головному потоці для Render
