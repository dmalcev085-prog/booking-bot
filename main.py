...
# Хелпер для безпечної відправки (запобігає крашу від помилки 403 / "bot was blocked")
def safe_send_message(chat_id, text, **kwargs):
    try:
        ...
        print(f"Невідома помилка відправки: {e}")
        return None
...
def index():
    return "✅ Telegram Booking Bot Server is active and running!", 200

# --- 5. МЕНЮ ТА КЛАВІАТУРИ ---
...
# --- 6. ОБРОБНИКИ КОМАНД І ПОВІДОМЛЕНЬ ---
...
def step_select_time(call):
    ...
    markup.add(types.KeyboardButton("❌ Скасувати"))
    safe_send_message(call.message.chat.id, f"<b>Крок 4/5:</b> Обрано {date_str}.\nОберіть час:", parse_mode="HTML", reply_markup=markup)
...
def finish_booking(message):
    ...
    # Сповіщення адміну (з безпечною відправкою, щоб уникнути крашу)
    if ADMIN_CHAT_ID:
...
# --- 7. АВТОМАТИЧНА РЕЄСТРАЦІЯ ВЕБХУКУ ---
def setup_webhook():
    try:
