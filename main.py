...
# --- ФОНОВИЙ ПОТІК НАГАДУВАНЬ КЛІЄНТАМ ---
def background_reminder_worker():
    """Кожні 10 хвилин перевіряє чи є активні записи на сьогодні і нагадує клієнтам"""
    while True:
        try:
            time.sleep(600)  # перевірка кожні 10 хв
            now_date = datetime.now().strftime("%Y-%m-%d")
            
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute("SELECT id, user_id, service, time FROM appointments WHERE date = ? AND status = 'ACTIVE' AND reminder_sent = 0", (now_date,))
            rows = cursor.fetchall()
            
            for row in rows:
                app_id, u_id, s_name, t_slot = row
                text = (
                    f"🔔 <b>НАГАДУВАННЯ ПРО ВІЗИТ!</b>\n\n"
                    f"Сьогодні у вас заплановано: <b>{s_name}</b> о <b>{t_slot}</b>.\n"
                    f"Чекаємо на вас у нашій перукарні! Якщо плани зміняться, скасуйте запис у «Моєму кабінеті»."
                )
                res = safe_send_message(u_id, text, parse_mode="HTML")
                if res:
                    cursor.execute("UPDATE appointments SET reminder_sent = 1 WHERE id = ?", (app_id,))
                    conn.commit()
            conn.close()
        except Exception as e:
            print(f"Помилка у фоновому потоці нагадувань: {e}")
...
def index():
    return "✅ Pro Barber Booking Bot with Reminders is Running smoothly!", 200
