...
# Окремий ендпоінт під токен для Telegram Webhook
...
# --- ВСТАНОВЛЕННЯ ПРАВИЛЬНОГО WEBHOOK ---
if __name__ == "__main__":
    bot.remove_webhook()
    if RENDER_EXTERNAL_URL:
        # Точна адреса з токеном на кінці
        webhook_url = f"{RENDER_EXTERNAL_URL.rstrip('/')}/{BOT_TOKEN}"
        bot.set_webhook(url=webhook_url)
        print(f"Correct Webhook set to: {webhook_url}")
