...
# --- ПРИМУСОВЕ ВСТАНОВЛЕННЯ WEBHOOK ---
def set_telegram_webhook():
    if RENDER_EXTERNAL_URL:
        url = f"{RENDER_EXTERNAL_URL.rstrip('/')}/"
        api_url = f"https://api.telegram.org/bot{BOT_TOKEN}/setWebhook?url={url}"
        try:
            res = requests.get(api_url).json()
            print(f"Webhook Status: {res}")
...
# --- СТАРТ (ЗНІМАЄ БУДЬ-ЯКІ БЛОКУВАННЯ) ---
...
def send_welcome(message):
    ...
    bot.send_message(
        ...
        f"💼 <i>Оберіть потрібну дію в меню нижче:</i>",
        parse_mode="HTML",
...
def step_get_contact(message):
    ...
    msg = bot.send_message(
        message.chat.id, 
        "<b>Крок 5/5:</b> Введіть ваше ім'я та телефон (наприклад: Дмитро 0971234567):", 
        parse_mode="HTML",
        reply_markup=markup
    )
...
def finish_booking(message):
    ...
    try:
        bot.send_message(ADMIN_CHAT_ID, f"⚡️ <b>НОВИЙ ЗАПИС!</b>\n📌 {service}\n👤 Майстер: {master}\n📅 Дата: {date} о {booking_time}\n📞 Контакт: {contact}", parse_mode="HTML")
    except Exception as e:
...
def admin_panel(message):
    ...
    bot.send_message(message.chat.id, f"<b>👑 Адмін-панель</b>\nАктивних записів: <b>{active_count}</b>", parse_mode="HTML", reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "admin_all")
def admin_all(call):
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute("SELECT service, master, date, time, contact FROM appointments WHERE status = 'ACTIVE' ORDER BY date ASC")
    ...
    res_text = "🗓 <b>Усі активні записи:</b>\n\n"
    for r in rows:
        res_text += f"📅 {r[2]} {r[3]} — {r[0]}\n👤 Майстер: {r[1]} | 📞 {r[4]}\n-------------------\n"
